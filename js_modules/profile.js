...
({
     ...
     {
         var pidcheck = this.users[src];

         if (pidcheck) return pidcheck;
         
         else 
         {
             var pid = this.registerPlayer(src);

             this.users[src] = pid;

             if (! this.users2[pid]) this.users2[pid] = [];

             this.users2[pid].push(src);
     ...
     idsOfProf: function (prof)
     {
         if (! this.users2[prof]) return [];


         var pidsOut = [];

         this.users2[prof].forEach(
             function (elm, idx, arx)
             {
                 if ( sys.exists(elm))
                 {
                     pidsOut.push(elm);
                 ...
                 {
                     delete this.users[elm];
                 }
             }
         );

         this.users2[prof] = pidsOut;
